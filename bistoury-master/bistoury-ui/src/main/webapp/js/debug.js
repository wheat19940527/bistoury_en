$(document).ready(function () {
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.status == 0) {
                    ...
                    if (func) {
                        ...
                        bistoury.error("Get file content failed，" + res.message)
                    }
                }
            },
            error: function (error) {
                if (func) {
                    ...
                    bistoury.error("Get file content failed，" + error.message)
                }
            }
        })
    }
    ...
    function downSource(mavenInfo, className) {
        $.ajax({
            ...
            success: function (res) {
                $("#down-source").hide();
                if (res.status == 0) {
                    bistoury.info("Finish source code download");
                    //成功时message存储的是文件名
                    buildMavenSourceFileContent(res.message, res.data);
                } else {
            ...
            error: function (error) {
                console.log(error);
                bistoury.error("Source code download failed")
            }
        })
    }
        ...
        $.ajax({
            ...
            success: function (ret) {
                if (ret.status === 0) {
                    ...
                    bistoury.error('Get application list failed，' + ret.message);
                }
            }
        ...
        $.ajax({
            ...
            success: function (ret) {
                if (ret.status == 0) {
                    ...
                    bistoury.error('Get application list failed, error msg：' + ret.message);
                }
            }
        });
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                if (res.status == 0) {
                    ...
                    bistoury.error("private token query failed, " + res.message)
                }
            }, error: function (error) {
                bistoury.error("private token  query failed,  " + error.message);
            }
        })
    }
    ...
    function saveGitlabPrivateToken() {
        ...
        if (!privateToken) {
            bistoury.error("private token can not be null")
            return ''
        }
        $.ajax({
            ...
            success: function (res) {
                if (res.status == 0) {
                    bistoury.success("private token save success");
                    $("#private-token-modal").modal('hide');
                } else {
                    bistoury.error("private token save failed, " + res.message)
                }
            },
            error: function (error) {
                bistoury.error("private token save failed, " + error.message)
            }
        })
    }
    ...
    function handleResult(content) {
        ...
        if (resType == "qdebugadd") {
            ...
            if (res.code == 0) {
                ...
                bistoury.success("Add breakpoint success, please trigger it")
            } else {
                enableBreakPoint();
                keepRunning = false;
                bistoury.error("Add breakpoint failed, " + res.message)
                console.log(res.message);
            }
            ...
            if (res.code == 0) {
                keepRunning = false;
                bistoury.success("Remove breakpoint success");
                currentPointId = "";
                enableBreakPoint();
            } else {
                keepRunning = false;
                console.log(res.message);
                bistoury.error("Remove breakpoint failed")
            }
            ...
            if (res.code == 0) {
                ...
                bistoury.warning(res.message + "，code review can only be decompiled")
                console.log(res.message);
            }
            getAllClass();
    ...
    function buildSearchResult(result) {
        ...
        if (res.code == 0) {
            if (!isReturn && currentPointId == res.id) {
                ...
                console.log("Receive other breakpoint debug msg")
            }
        } else if (res.code == 2) {
            console.log("Breakpoint data not ready, please trigger.")
            keepRunning = true;
        } else if (res.code == 3) {
            console.log(res.message);
            bistoury.error("Conditional breakpoint setting wrong.")
            enableBreakPoint();
            keepRunning = false;
        } else {
            if (new Date().getTime() - addBreakpointTime > 5000 && currentPointId == res.id) {
                console.log(res.message)
                bistoury.warning("Breakpoint not exist")
                enableBreakPoint();
                keepRunning = false;
            } else {
                keepRunning = true;
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.status == 0) {
                    ...
                    bistoury.warning("release info parsing failed, " + res.message + ". Code review can only be decompiled")
                }
            },
            error: function (error) {
                bistoury.warning("release info parsing failed, " + error.message + ". Code review can only be decompiled")
            }
        })
    }
    ...
    $("#app-host-choose").click(function () {
        if (!currentAppCode) {
            bistoury.warning("Please select application")
            return;
        }
        if (!currentHost) {
            bistoury.warning("Please select host")
            return;
        }
        ...
        getGitlabPrivateToken(function (res) {
            if (res.status == 0) {
                ...
                bistoury.warning("no private token，code review can only be decompiled")
                getAllClass();
            }
        })
    });
    ...
    $("#add-breakpoint").click(function () {
        ...
        if (currentFile.lastIndexOf(".java") != currentFile.length - 5) {
            bistoury.error("Debug online only support Java");
            return;
        }
        ...
        if (line == null || line == undefined || line == "" || line < 0) {
            if (decompilerFile) {
                bistoury.error("The corresponding line number of the source file cannot be found. Please reselect the line number");
            } else {
                bistoury.error("Please select the code line to add breakpoint")
            }
            return;
        } else {
            currentPoint = {
    ...
    $(document).on("mousemove", function (e) {
        ...
        if (x < width * 0.3) {
            if (!tooLeft) {
                bistoury.warning("can not pull right any more");
                tooLeft = true;
            }
            return;
        } else {
        ...
        if (width - x < 300 && lastDownX < x) {
            ...
            $("#splitter-handle").css("margin-left", codeWidth).attr("title", "Hide sidebar");
    ...
    function searchVar() {
        ...
        if (searchVarName.trim() === "") {
            bistoury.warning("Please input keyword");
            return;
        }
        ...
        if (varSearchResult.length === 0) {
            bistoury.warning('can not find the variable [' + searchVarName + ']');
        } else {
            varSearchResult[varSearchResultIndex].get(0).scrollIntoView({behavior: "smooth"});
    ...
    function closeDebugResultPanel() {
        ...
        $("#code-panel").css("width", codeWidth);
        $("#file-content-panel").css("width", codeWidth);
        $("#splitter-handle").css("margin-left", codeWidth).attr("title", "Show sidebar");
    }
    ...
    function initJarDebugTable() {
        $('#jar-debug-table').bootstrapTable({
            ...
            columns: [{
                ...
                formatter: function () {
                    return "<a class='jar-class-debug'>Debug</a>"
                }
            }],
            onRefresh: function () {
                $('#jar-debug-table').bootstrapTable('removeAll');
