$(document).ready(function () {
    ...
    function buildThreadInfo(jvmInfo) {
        ...
        threadCurve.setOption({
            title: {
                text: "Thread",
                subtext: "Active Thread: " + jvmInfo.currentThreadCount + " Daemon Thread: " + jvmInfo.daemonThreadCount
                    + " Start Count: " + jvmInfo.totalStartedThreadCount + " Peak: " + jvmInfo.peakThreadCount
            },
            xAxis: {
                data: threadTimeAxis
            },
    ...
    function buildHeapHisto(heapHistos) {
        if (!heapHistos || heapHistos.length <= 0) {
            bistoury.warning("No data found");
            return;
        }
        ...
        $.ajax({
            ...
            success: function (ret) {
                if (ret.status === 0) {
                    ...
                    bistoury.error('Get application failed');
                }
            }
        ...
        $.ajax({
            ...
            success: function (ret) {
                if (ret.status == 0) {
                    ...
                    bistoury.error('Get host list failed, error msg:' + data.message);
                }
            }
        });
    ...
    $("#jvm-heap-histo-menu").click(function () {
        ...
        if (!firstIn.heapHisto) {
            ...
            $("#heap-histo-time").val("")
            bistoury.info("Refresh and query data");
            //getHeapHisto();
        }
    });
    ...
    $("#heap-histo-search").click(function () {
        ...
        bistoury.warning("The function can not be use")
    })
    ...
    $("#thread-max-depth").on("keyup", function () {
        ...
        if (maxDepth < 0) {
            ...
            alert("max depth no less than 0")
        }
    });
    ...
    window.setInterval(function () {
        if (keepRunning) {
            ...
            if (now - startTime < keepTime) {
                ...
                keepRunning = false;
                bistoury.keepError("Run time out " + minutes + " mins，stop query", function () {
                    var continueRun = confirm("continue query");
                    if (continueRun) {
                        keepRunning = true;
                        startTime = new Date().getTime();
                    }
