...
function init(info) {
    if (info == null) {
        bistoury.error("Failed to get performance analysis information");
        return;
    }
    var profiler = info.profiler;
    $("#profiler_mode_info").html("Analysis mode: " + getMode(profiler.mode));
    if (profiler.mode === async_sampler) {
        $("#event_type").html("Analysis mode: " + info.eventType);
        $("#event_type").css("display", "");
    }
    $("#start_time").html("Start: " + profiler.startTime)
    $("#duration").html("Actual performance analysis time:" + info.realDuration + "s")
    $("#default_duration").html("Expect performance analysis time:" + profiler.duration + "s");
    $("#default_interval").html("Expect performance analysis interval:" + profiler.interval + "ms");
