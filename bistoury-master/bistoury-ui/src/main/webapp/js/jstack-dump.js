...
var legend = {
    data: ['Total CPU Proportion', 'Thread Count', 'Selected CPU Proportion']
}
var yDatas = []
var xDatas = []
...
function fillThreadInfo(data) {
    ...
    threadNameContext['thread'].push({value: '', name: 'Total threads'})
...
function fillEchart(data) {
    ...
    if (isTotalThreads(data)) {
        ...
        options.series[0] = getSeries('Total CPU Proportion', yDatasInterval, 0)
        ...
        options.series[1] = getSeries('Thread Count', yDatasInterval, 1)
...
function dataFilter() {
    ...
    $('#label_minute_total_cpu_time').text('CPU usage per minute: ' + minuteTotalCPUTime.toFixed(2) + ' %')
    $('#label_moment_total_cpu_time').text('Current CPU usage: ' + momentTotalCPUTime.toFixed(2) + ' %')
    $('#all-jstack-table').bootstrapTable('removeAll')
...
function getEchartOption() {
    return {
        title: {
            text: 'cpu proportion',
            subtext: 'Click the corresponding point to view the information\n\n(you need to open the jstack switch\n in the application center before using)'
        },
        tooltip: {
            trigger: 'axis'
        },
...
$(document).ready(function () {
    ...
    $('#thread-select').on('change', function () {
        ...
        threadSelectName = $(this).find('option:selected').text() + 'Thread CPU proportion'
    })
    dataRangeRefresh()
...
var send = function (machine, type, input) {
    ...
    if (machine == null) {
        bistoury.error("Please select machine");
