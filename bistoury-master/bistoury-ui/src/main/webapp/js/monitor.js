$(document).ready(function () {
    ...
    function fillData() {
        _monitor.quotas.forEach(function (quota) {
            ...
            if (_monitor.curve[quota].type == quotaType.timer) {
                ...
                bistoury.error(quota + " The monitoring indicator type is wrong, please contact admin!");
                return;
            }
    ...
    function drawCurve(curve, title, legend, xAxis, yAxis) {
        if (_monitor.interval != -1) {
            title += "---interval" + _monitor.interval + "min";
        }
        curve.setOption({
            title: {
                text: title,
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.status == 0) {
                    ...
                    if (func) {
                        ...
                        bistoury.error("Failed to get file contents, " + res.message)
                    }
                }
            },
            error: function (error) {
                if (func) {
                    ...
                    bistoury.error("Failed to get file contents, " + error.message)
                }
            }
        })
    }
    ...
    function downSource(mavenInfo, className) {
        $.ajax({
            ...
            success: function (res) {
                $("#down-source").hide();
                if (res.status == 0) {
                    bistoury.info("Source code download success");
                    //成功时message存储的是文件名
                    buildMavenSourceFileContent(res.message, res.data);
                } else {
    ...
    function saveGitlabPrivateToken() {
        ...
        if (!privateToken) {
            bistoury.error("private token can't be null")
            return ''
        }
        $.ajax({
        ...
        if (resType == "qmonitorquery") {
            ...
            if (res.status == 0) {
                if (res.data.length == 0) {
                    bistoury.warning("No monitoring data is found in the current time interval. Please modify the query criteria.");
                    keepRunning = false;
                } else {
            ...
            var res = result.data;
            if (res.code == 0) {
                bistoury.success("Monitor add success")
            } else {
                bistoury.error("Breakpoint add success " + res.message)
                console.log(res.message);
            }
            ...
            if (res.code == 0) {
                ...
                console.log(res.message);
                bistoury.warning(res.message + ", code review can be decompiled")
            }
            getAllClass();
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.status == 0) {
                    ...
                    bistoury.warning("release info parse failed, " + res.message + ". Code review can only be decompiled")
                }
            },
            error: function (error) {
                bistoury.warning("release info parse failed, " + res.message + ". Code review can only be decompiled")
    ...
    $("#app-host-choose").click(function () {
        ...
        getGitlabPrivateToken(function (res) {
            if (res.status == 0) {
                ...
                bistoury.warning("Please set private token, code review can only be decompiled")
                getAllClass();
            }
        })
    });
    ...
    $("#add-monitor").click(function () {
        ...
        if (line == null || line == undefined || line == "" || line < 0) {
            if (decompilerFile) {
                bistoury.error("The corresponding line number of the source file cannot be found. Please re-select the line number");
            } else {
    ...
    $(document).on("mousemove", function (e) {
        ...
        if (x < width * 0.4) {
            if (!tooLeft) {
                bistoury.warning("Can't pull left anymore");
                tooLeft = true;
            }
            return;
        } else {
    ...
    function initJarDebugTable() {
        $('#jar-debug-table').bootstrapTable({
            ...
            columns: [{
                ...
                formatter: function () {
                    return "<a class='jar-class-monitor'>Monitor</a>"
                }
            }],
            onRefresh: function () {
                $('#jar-debug-table').bootstrapTable('removeAll');
