...
$(document).ready(function () {
    ...
    $(function () {
        ...
        function addApps(apps) {
            ...
            var select = $("<select><option value=''></option></select>").attr("data-placeholder", "Please select application").attr("id", "app-select");
            ...
            $.ajax({
                ...
                success: function (ret) {
                    if (ret.status === 0) {
                        ...
                        bistoury.error("Get admin application list failed");
                    }
                }
            });
        }
            ...
            $.ajax({
                ...
                success: function (ret) {
                    if (ret.status == 0) {
                        ...
                        bistoury.error('Get application list failed, error msg: ' + data.message);
                    }
                }
            });
        }
        ...
        function initTabContent(appCode, hosts) {
            ...
            var addHosts = function () {
                ...
                if (hosts.length >= 0) {
                    var placeholder;
                    if (hosts.length == 0) {
                        placeholder = "The application center didn't find the machine belonging to the application";
                    } else {
                        placeholder = "Please select a machine. If it is not selected, all machines are selected (tail, grep, zgrep, ls, head commands support multi machine execution, and other commands need to select a machine";
                    ...
                    var fullScreenBtn = $("<button></button>").addClass("btn btn-info btn-sm").css("float", "right").css("height", "32px").css("max-width", "55px").append("Full-screen");
                    var isFull = false;
                    fullScreenBtn.click(function () {
                        bistoury.info("Esc exit full screen")
                        isFull = true;
                    ...
                    hostsHtml += '<input id="hosts-' + tid + '" type="checkbox" autocomplete="off" name="host" value="" checked="checked">' +
                        '<label for="hosts-' + tid + '" class="cur">All</label>';
